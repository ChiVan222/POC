...
def run_pipeline(image_path):
    ...
    for i in range(len(objects)):
        for j in range(len(objects)):
            ...
            geo_vec, dist = get_geometry_vector(
                subj["box"], obj["box"], w_img, h_img, subj["z"], obj["z"]
            )

            bx1, by1, bw1, bh1 = subj["box"]
            bx2, by2, bw2, bh2 = obj["box"]
            is_overlap = (
                max(bx1, bx2) < min(bx1 + bw1, bx2 + bw2)
                and max(by1, by2) < min(by1 + bh1, by2 + bh2)
            )
            ...
            with torch.no_grad():
                sl, al, *_ = sgg_model(
                    geo_vec.to(DEVICE).unsqueeze(0),
                    vis_feats[i].unsqueeze(0),
                    s_anchors,
                    a_anchors
                )
            ...
            s_val, s_id = s_prob.max(1)
            a_val, a_id = a_prob.max(1)

            if s_val > RELATION_CONF_THRESH:
                all_predictions.append({
                    "s": subj["label"],
                    "p": SPATIAL_CLASSES[s_id.item()],
                    "o": obj["label"],
                    "score": s_val.item(),
                    "boxes": (subj["box"], obj["box"])
                })

            if a_val > RELATION_CONF_THRESH:
                all_predictions.append({
                    "s": subj["label"],
                    "p": ACTION_CLASSES[a_id.item()],
                    "o": obj["label"],
                    "score": a_val.item(),
                    "boxes": (subj["box"], obj["box"])
                })
...
def visualize_pipeline(img_pil, relations):
    ...
    for rel in relations:
        ...
        sb, ob = rel["boxes"]
        s_c = (sb[0] + sb[2] / 2, sb[1] + sb[3] / 2)
        o_c = (ob[0] + ob[2] / 2, ob[1] + ob[3] / 2)
...
if __name__ == "__main__":
    ...
    if res:
        visualize_pipeline(res[0], res[1])
