...
ID_TO_CATEGORY = {VG150_PREDICATES.index(p): cat for cat, plist in FULL_MAPPING.items() 
...
@torch.no_grad()
def evaluate_official_constrained_predcls():
    print(f"Evaluating OFFICIAL Graph-Constrained PredCls on {DEVICE}")
    ...
    cat_hits = {cat: {k: 0 for k in K_VALS} for cat in FULL_MAPPING}
    cat_totals = {cat: 0 for cat in FULL_MAPPING}
    ...
    for idx in tqdm(range(len(vg_base)), desc="Evaluating"):
        if str(idx) not in h5_f: continue
        grp = h5_f[str(idx)]
        ...
        for k in K_VALS:
            for r_idx in ranking[:k]:
                ...
                if rel in gt_relations:
                    cat = ID_TO_CATEGORY.get(rel[1], "Structural/Misc")
                    cat_hits[cat][k] += 1
                    per_pred_hits[rel[1]][k] += 1
    ...
    for k in K_VALS:
        ...
        mr_row.append(f"{np.mean(recalls):<10.4f}" if recalls else f"0.0000")
    print(" | ".join(mr_row + ["-"]))

    total_gt = sum(cat_totals.values())
    total_row = [f"{'TOTAL RECALL (R@K)':<20}"]
    for k in K_VALS:
        hits_sum = sum(cat_hits[c][k] for c in cat_hits)
        total_row.append(f"{hits_sum / total_gt:<10.4f}" if total_gt else f"0.0000")
    print(" | ".join(total_row + [str(total_gt)]))
    print("="*90)
    h5_f.close()

if __name__ == "__main__":
    evaluate_official_constrained_predcls()